...
pub fn resolve_or_allocate(
    ...
    match reference {
        ...
        ConceptRef::Label(label) => {
            if let Some(row) = state.label_to_id.get(label).copied() {
                let resolved = state.resolve_alias(row);
                anyhow::ensure!(
                    state.is_allocated(resolved),
                    "label {label} resolves to freed row"
                );
                return Ok(resolved);
            }
...
fn allocate_label(
    ...
    state.label_to_id.insert(label.to_string(), row);

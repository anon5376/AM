...
pub fn run_core_criteria(theta: &Theta) -> Vec<CriterionResult> {
    run_core_criteria_with_config(theta, CriteriaConfig::full_scale())
}

pub fn run_core_criteria_with_config(
    theta: &Theta,
    config: CriteriaConfig,
) -> Vec<CriterionResult> {
    vec![
        determinism(theta),
        completion_with_config(theta, config),
        reinforcement(theta),
        contradiction(theta),
        forgetting(theta),
...
pub fn completion(theta: &Theta) -> CriterionResult {
    completion_with_config(theta, CriteriaConfig::full_scale())
}
...
pub fn completion_with_config(theta: &Theta, config: CriteriaConfig) -> CriterionResult {
    ...
    for assembly in 0..config.completion_assemblies {
        let labels = (0..5)

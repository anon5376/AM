...
pub fn update_contradictions(
    ...
    for (idx, contradiction) in state.open_contradictions.iter().enumerate() {
        if contradiction.status == ContradictionStatus::Open
            && state
                .resolve_row_ref(contradiction.concept)
                .map(|row_ref| state.v_get(row_ref.id, contradiction.axis) < state.theta.th_v_close)
                .unwrap_or(false)
        {
    ...
    for idx in close_indices {
        let row = state.open_contradictions[idx].concept.id;
...
fn signs_alternate(state: &AmState, row: usize, axis: usize) -> bool {
    let Some(history) = state.recent_writes.get(&(state.row_ref(row), axis)) else {
        return false;
    };
    if history.len() < 4 {
        return false;

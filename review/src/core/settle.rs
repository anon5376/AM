...
pub fn settle(state: &mut AmState, external_input: &[f32], trace: &mut StepTrace) -> SettleResult {
    ...
    for iter in 0..state.theta.t {
        ...
        for row in &rows {
            ...
            let input = external_input.get(*row).copied().unwrap_or(0.0);
            let sigmoid = 1.0 / (1.0 + (-(net + input - state.theta.th0)).exp());
